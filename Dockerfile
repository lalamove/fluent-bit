...
RUN apt-get update \
    && apt-get install -y \
       ...
       libsystemd-dev \
    && cd /tmp/src/build/ \
    && cmake -DFLB_DEBUG=Off -DFLB_TRACE=Off -DFLB_JEMALLOC=On -DFLB_BUFFERING=On -DFLB_TLS=On -DFLB_WITHOUT_SHARED_LIB=On -DFLB_WITHOUT_EXAMPLES=On ../ \
    && make \
    && install bin/fluent-bit /fluent-bit/bin/
# Configuration files
COPY conf/fluent-bit.conf /fluent-bit/etc/
COPY conf/parsers.conf /fluent-bit/etc/
COPY conf/parsers_java.conf /fluent-bit/etc/
